...
Before searching the codebase to locate a module or understand what it does, always
read `./ARCHITECTURE.md` first. It is the mental map of this codebase: module
responsibilities, dependency boundaries, and architecture invariants.
...
On the interactive path: isolate with the same flags as headless (`--no-session`, `--no-extensions -e <ext>`, `--no-skills --skill <skill>` as needed), and start pi through the `pi-headless` SKILL's `piw` wrapper WITHOUT `-p`/`--print`/`--mode json` (e.g. `piw --no-session --no-extensions -e <ext>`); piw with no print/json mode starts the interactive TUI. piw injects `--model`/`--thinking` itself; never pass those flags. Drive input via tmux send-keys only after the pane is ready.

To cleanly quit pi interactive mode during testing, send the slash command `/quit` (Ctrl+C or Ctrl+D may leave the session in a bad state or not fully terminate the TUI). After quitting, relaunch with explicit extension loading e.g. `piw --no-session --no-extensions -e ./pi/agent/extensions/inline.ts` (or the relevant extension) so that code changes take effect. Use a dedicated clean pane or window for each test run to avoid side effects on other work.
