...
pi coding agent 的自定义 TypeScript 扩展集。通过 pi 的 ExtensionAPI 注册 TUI 斜杠命令（`/context`、
`/watch`、`/files`、`/review`、`/answer`、`/handoff`、`/loop`、`/notify`），扩展 agent 的原生能力，覆盖文件管理、
会话控制、代码审查、提示词编辑、会话交接与系统通知。依赖 `@mariozechner/pi-coding-agent` 与
`@mariozechner/pi-tui` 运行时库。
...
**API Boundary:** `models.json` 与 `modes.json` 是模型切换的唯一入口；`extensions/` 不直接读取模型配置。
