...
Use `{baseDir}/scripts/ksql-run.sh` to query Kingbase databases. The script reads connection credentials from `.env` in the current working directory and handles the connection setup.
...
```sh
{baseDir}/scripts/ksql-run.sh -c "\\d+t"
{baseDir}/scripts/ksql-run.sh -c "SELECT * FROM schema.table_name LIMIT 10;"
```
...
```sh
{baseDir}/scripts/ksql-run.sh -c "\\d+ <table>"
```

Use a schema-qualified table name when available:

```sh
{baseDir}/scripts/ksql-run.sh -c "\\d+ public.my_table"
```

### Execute SQL

```sh
{baseDir}/scripts/ksql-run.sh -c "SELECT * FROM <table> LIMIT 100;"
```
...
```sh
{baseDir}/scripts/ksql-run.sh -c "UPDATE <table> SET <column> = <value> WHERE <condition>;"
```
...
```sh
{baseDir}/scripts/ksql-run.sh -c "\\df"
```

```sh
{baseDir}/scripts/ksql-run.sh -c "\\df+ schema.name"
```
...
1. Never print `KB_PWD` or the full `DATABASE_PARAM` in the final answer.
