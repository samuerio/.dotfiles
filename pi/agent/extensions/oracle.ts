/**
 * Oracle subagent extension (sibling file): specialized spec + tool
 * registration, reusing the Subagent machinery from `lib/subagent.ts`.
 *
 * Registers one native pi tool:
 *   - `oracle` : specialized reasoning-advisor subagent (baked-in spec) for
 *     planning, code review, and deep technical questions; child runs with
 *     read + bash only.
 */
