...
const getActiveClientWindowIds = async (
    pi: ExtensionAPI,
): Promise<string[]> => {
...
const getWindowIdFromPaneId = async (
    pi: ExtensionAPI,
    paneId: string,
): Promise<string | null> => {
    const { stdout, code } = await pi.exec("tmux", [
...
const markWindowAsUnread = async (
    pi: ExtensionAPI,
    windowId: string,
): Promise<boolean> => {
    // Set @unread option
    const { code: setCode } = await pi.exec("tmux", [
        "set",
        "-w",
        "-t",
        windowId,
    ...
    const { code: refreshCode } = await pi.exec("tmux", [
        "refresh-client",
        "-S",
    ]);
