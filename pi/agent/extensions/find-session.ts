...
export const SearchSessionsParams = Type.Object({
	...
	since: Type.Optional(
		Type.String({
			description: "Only messages with timestamp >= this ISO time. Bounds the MESSAGE timestamp, not file mtime.",
		}),
	),
	until: Type.Optional(
		Type.String({
	...
	includeCurrentSession: Type.Optional(
		Type.Boolean({
			description: "Include the live session in the search. Default false (excluded).",
		}),
	),
});
 ...
 */
function toolCallHaystack(msg: Extract<AgentMessage, { role: "assistant" }>): string {
	const blocks: string[] = [];
	for (const part of msg.content) {
		...
		blocks.push(`[tool: ${part.name} ${preview(json, TOOL_CALL_ARGS_PREVIEW)}]`);
	}
	return blocks.join("\n");
}
...
function validateMaxResults(max: number | undefined): number {
	...
	if (!Number.isFinite(max) || !Number.isInteger(max) || max < 1) {
		throw new Error(`search_sessions: invalid maxResults ${max} (must be a positive integer).`);
	}
...
export function parseSearchArgs(args: string): {
			...
			switch (flag) {
				case "role":
					if (value !== "user" && value !== "assistant") {
						throw new Error(`--role must be user or assistant (got "${value ?? ""}").`);
					}
					role = value;
					break;
				case "since":
				...
				case "include-tool-calls":
					...
					break;
				default:
					throw new Error(`unknown flag --${flag} (supported: role, since, until, max, include-tool-calls).`);
			}
			continue;
		}
	...
	if (!query) throw new Error("usage: /search-sessions [--role=user|assistant] [--since=ISO] [--until=ISO] [--max=N] [--include-tool-calls] <query>");
 ...
 */
export async function searchSessions(options: SearchSessionsOptions): Promise<SearchSessionsResult> {
	...
	const untilMs = options.until !== undefined ? Date.parse(options.until) : undefined;
	if (sinceMs !== undefined && Number.isNaN(sinceMs)) throw new Error(`search_sessions: invalid since "${options.since}".`);
	if (untilMs !== undefined && Number.isNaN(untilMs)) throw new Error(`search_sessions: invalid until "${options.until}".`);
	const includeToolCalls = options.includeToolCalls === true;
	const roleFilter = options.role;

	const sessions = await SessionManager.list(process.cwd());
	const currentAbs = options.currentSessionFile ? path.resolve(options.currentSessionFile) : undefined;
	...
	for (const session of sessions) {
		...
		if (currentAbs && path.resolve(session.path) === currentAbs && options.includeCurrentSession !== true) {
			continue;
		}
		...
		if (stat.size > MAX_SESSION_FILE_BYTES) {
			skippedFiles.push(`${shortenPath(session.path)} (${Math.round(stat.size / 1024)} KB)`);
			continue;
		}
		...
		for (const entry of contextEntries) {
			...
			for (const msg of sessionEntryToContextMessages(entry)) {
				...
				if (haystack.length > MAX_HAYSTACK_CHARS) haystack = haystack.slice(0, MAX_HAYSTACK_CHARS);
				...
				if (sinceMs !== undefined && !(Number.isFinite(tsMs) && tsMs >= sinceMs)) continue;
				if (untilMs !== undefined && !(Number.isFinite(tsMs) && tsMs <= untilMs)) continue;
...
export function formatHits(result: SearchSessionsResult, query: string): string {
	const lines: string[] = [];
	...
	const envelopeParts = [
		...
		result.truncated ? `truncated=true (maxResults reached; narrow the query or raise maxResults)` : undefined,
		result.skippedFiles.length ? `skipped=${result.skippedFiles.length} (${result.skippedFiles.join(", ")})` : undefined,
...
function searchCallComponent(theme: any, context: any, title: string, muted: string) {
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		renderCall(args, theme, context) {
			...
			return searchCallComponent(theme, context, "search_sessions", preview(query, 80) || "?");
		},
		...
		async execute(
			...
			try {
				...
				throw new Error(`search_sessions: ${error instanceof Error ? error.message : String(error)}`);
			}
		},
