...
async function handleModeChoiceUI(
    ...
    if (
        runtime.currentMode === CUSTOM_MODE_NAME &&
        choice !== CUSTOM_MODE_NAME
    ) {
        const action = await ctx.ui.select(`Mode \"${choice}\"`, [
            "use",
            "store",
        ]);
        if (!action) return;
...
function createModelRuntimeFromRegistry(
    ...
    return {
        ...
        refresh: async (options) => {
            const result = await registry.refresh(options as any);
            if (result && typeof (result as any).errors !== "undefined") return result as any;
    ...
    return ctx.ui.custom<{ provider: string; modelId: string } | undefined>(
        (tui, _theme, _keybindings, done) => {
            const selector = new ModelSelectorComponent(
                tui,
                currentModel,
                createModelRuntimeFromRegistry(ctx.modelRegistry) as any,
                scopedModels as any,
                (model: any) =>
...
async function pickThinkingLevelForModeUI(
    ...
    const ordered = [
        defaultValue,
        ...options.filter((x) => x !== defaultValue),
    ];
...
class PromptEditor extends CustomEditor {
    ...
    render(width: number): string[] {
        ...
        let labelLen = labelParts.reduce(
    ...
    pi.registerCommand("mode", {
        ...
        handler: async (args, ctx) => {
            ...
            if (tokens[0] === "store") {
                ...
                if (ctx.hasUI)
                    ctx.ui.notify(
                        `Stored current selection into "${target}"`,
                        "info",
                    );
                return;
            }
