...
function getReviewSettings(ctx: ExtensionContext): ReviewSettingsState {
    ...
    for (const entry of ctx.sessionManager.getEntries()) {
        if (
            entry.type === "custom" &&
            entry.customType === REVIEW_SETTINGS_TYPE
        ) {
...
function hasNeedsAttentionVerdict(messageText: string): boolean {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (heading) {
            const normalizedHeading = heading.title
                .replace(/[*_`]/g, "")
                .trim();
        ...
        for (let j = i + 1; j < lines.length; j++) {
            ...
            if (nextHeading) {
                ...
                if (
                    verdictLevel === null ||
                    nextHeading.level <= verdictLevel
                ) {
                    break;
                }
                if (
...
export default function reviewExtension(pi: ExtensionAPI) {
        ...
        while (true) {
                ...
                (tui, theme, _kb, done) => {
                    ...
                    container.addChild(
                        new Text(
                            theme.fg(
                                "accent",
                                theme.bold("Select a review preset"),
                            ),
                        ),
                    );
            ...
            if (result === TOGGLE_CUSTOM_INSTRUCTIONS_VALUE) {
                ...
                if (!customInstructions?.trim()) {
                    ctx.ui.notify(
                        "Custom review instructions not changed",
                        "info",
                    );
                    continue;
                }
    ...
    async function showBranchSelector(
        ...
        const result = await ctx.ui.custom<string | null>(
            (tui, theme, keybindings, done) => {
                ...
                container.addChild(
                    new Text(
                        theme.fg("accent", theme.bold("Select base branch")),
                    ),
                );
                ...
                const updateList = () => {
                    ...
                    if (filteredItems.length === 0) {
                        listContainer.addChild(
                            new Text(
                                theme.fg("warning", "  No matching branches"),
                            ),
                        );
                        selectList = null;
                        return;
    ...
    async function showCommitSelector(
        ...
        const result = await ctx.ui.custom<{
            ...
            container.addChild(
                new Text(
                    theme.fg("accent", theme.bold("Select commit to review")),
                ),
            );
    ...
    async function showPrInput(
        ...
        if (!checkoutResult.success) {
            ctx.ui.notify(
                `Failed to checkout PR: ${checkoutResult.error}`,
                "error",
            );
            return null;
        }
    ...
    async function runLoopFixingReview(
        ...
        try {
            ...
            for (let pass = 1; pass <= REVIEW_LOOP_MAX_ITERATIONS; pass++) {
                const reviewBaselineAssistantId =
                    getLastAssistantSnapshot(ctx)?.id;
                ...
                if (reviewSnapshot.stopReason === "aborted") {
                    ctx.ui.notify(
                        "Loop fixing stopped: review was aborted.",
                        "warning",
                    );
                    return;
                }
    ...
    pi.registerCommand("review", {
        ...
        handler: async (args, ctx) => {
            ...
            if (reviewLoopInProgress) {
                ctx.ui.notify(
                    "Loop fixing review is already running.",
                    "warning",
                );
                return;
            }
