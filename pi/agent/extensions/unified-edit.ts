...
type Preview =
    | { diff: string; files: string[]; firstChangedLine?: number }
    | { error: string };
...
function findMatchIndex(
    ...
    while (index !== -1) {
        if (
            !wholeLines ||
            isWholeLineBoundary(content, index, needle.length, needle)
        )
            return index;
...
function countNeedleOccurrences(
    content: string,
    needle: string,
    wholeLines: boolean,
): number {
    ...
    while (index !== -1) {
        ...
        index = content.indexOf(
            needle,
            index + (wholeLines ? 1 : needle.length),
        );
    }
    return count;
}
...
async function readExistingNormalized(
    ...
    try {
        return normalizeToLF(
            stripBom(await readFile(absolutePath, "utf-8")).text,
        );
    } catch (err: any) {
        const code =
...
function parseRowScript(text: string): RawFileScript[] {
    ...
    function finishOp() {
        if (!currentOp) return;
        if (!currentFile)
            throw new Error("Internal parser error: operation without file.");
        if ("rows" in currentOp && currentOp.rows.length === 0) {
            throw new Error(
                `${currentOp.kind} in ${currentFile.path} has no + rows.`,
            );
        }
            ...
            const opName =
                currentOp.kind === "replace"
                    ...
                    : currentOp.kind === "insertBeforeAnchor"
                      ...
                      : "@INS.AFTER";
            throw new Error(
                `${opName} in ${currentFile.path} has no + or - rows.`,
            );
        }
    ...
    function pushGroup(marker: RowGroup["marker"], linesToAdd: string[]): void {
        if (!currentOp || !("groups" in currentOp))
            throw new Error(
                "Internal parser error: group row without group operation.",
            );
        if (marker === "@@") {
        ...
        if (lastGroup && lastGroup.marker === marker)
            lastGroup.lines.push(...linesToAdd);
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (raw.startsWith("@")) {
            ...
            if (insertMatch) {
                ...
                if (!Number.isSafeInteger(line) || line < 1)
                    throw new Error(
                        `Line ${lineNumber}: insert line number must be >= 1.`,
                    );
                currentOp =
            ...
            const delMatch =
                /^@DEL\s+(\d+)(?:(?:\s*-\s*|\s*\.\.=?\s*|\s*\.=\s*)(\d+))?\s*$/i.exec(
                    trimmed,
                );
            if (delMatch) {
                const startLine = Number(delMatch[1]);
                const endLine =
                    delMatch[2] === undefined ? startLine : Number(delMatch[2]);
        ...
        if (raw.startsWith("+") || raw.startsWith("-")) {
            ...
            if (!currentOp)
                throw new Error(
                    `Line ${lineNumber}: row appears before an operation.`,
                );
            ...
            if ("rows" in currentOp) {
                if (marker !== "+")
                    throw new Error(
                        `Line ${lineNumber}: ${currentOp.kind} only accepts + rows.`,
                    );
...
function getContextualReplacePairs(
    ...
    for (let i = 0; i < hunks.length; i++) {
        ...
        if (!hasChange)
            throw new Error(`@REPLACE${label} in ${path} has no + or - rows.`);
...
function getReplacePairs(
    ...
    const groups = op.groups.filter(
        (group) => group.marker === "@@" || group.lines.length > 0,
    );
    if (groups.length === 0)
        throw new Error(`@REPLACE in ${path} has no rows.`);
    ...
    if (changeGroups.length === 1) {
        ...
        throw new Error(
            `@REPLACE in ${path} has + rows but no - rows to locate the replacement.`,
        );
    }
...
function applyAnchorInsertOperation(
    ...
    op: Extract<
        RawRowOperation,
        { kind: "insertBeforeAnchor" | "insertAfterAnchor" }
    >,
): string {
    const opName =
        op.kind === "insertBeforeAnchor" ? "@INS.BEFORE" : "@INS.AFTER";
    const groups = op.groups.filter(
    ...
    const newText =
        op.kind === "insertBeforeAnchor"
            ? `${insertText}\n${anchorText}`
            : `${anchorText}\n${insertText}`;
    return applyEditsToNormalizedContent(
        content,
...
async function buildRowPlan(text: string, cwd: string): Promise<ParsedPlan> {
    ...
    for (const script of scripts) {
        ...
        if (snapshot.current === null)
            throw new Error(`Cannot edit deleted file ${script.path}.`);
        snapshot.current = applyRowOperations(
            script.path,
            snapshot.current,
            script.ops,
        );
    ...
    return {
        mode: "rows",
        changes: store.collectChanges(
            "The row edit script produced no changes.",
        ),
    };
}
...
function patchTextForPreview(text: string): string {
    ...
    return normalized.endsWith("*** End Patch")
        ? normalized
        : `${normalized}\n*** End Patch`;
}
...
async function buildPatchPlan(text: string, cwd: string): Promise<ParsedPlan> {
    ...
    for (const op of operations) {
        ...
        if (snapshot.current === null)
            throw new Error(
                `Failed to update ${op.path}: file does not exist.`,
            );
        snapshot.current = deriveUpdatedContent(
            op.path,
            snapshot.current,
            op.chunks,
        );
    ...
    return {
        mode: "patch",
        changes: store.collectChanges("The patch produced no changes."),
    };
}

async function buildPlan(text: string, cwd: string): Promise<ParsedPlan> {
    return isPatchLikePayload(text)
        ? buildPatchPlan(text, cwd)
        : buildRowPlan(text, cwd);
}
...
async function applyAddChange(
    ...
    return withFileMutationQueue(change.absolutePath, async () => {
        ...
        if (existing !== null)
            throw new Error(
                `Could not add ${change.path}: file already exists.`,
            );
...
function combineDetails(
    files: UnifiedEditDetails["files"],
): UnifiedEditDetails {
    const diff =
        files.length === 1
...
function extractRowHeaderPaths(text: string): string[] {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const partial =
            i === lines.length - 1 ? /^\[([^\]]+)$/.exec(trimmed) : null;
...
function getRenderablePaths(text: string | undefined): string[] | undefined {
    ...
    try {
        const paths = patchLike
            ? parsePatch(
                  isPatchPayload(text) ? text : patchTextForPreview(text),
              ).map((op) => op.path)
...
function renderUnifiedPathLabel(
...
function buildUnifiedEditCallComponent(
    ...
    component.addChild(
        new Text(
            formatUnifiedEditCall(text, component.preview, theme, cwd),
            0,
            0,
        ),
    );
...
export default function unifiedEditExtension(pi: ExtensionAPI) {
    pi.registerTool({
        ...
        async execute(
            ...
            return {
                content: [
                    { type: "text" as const, text: formatSummary(details) },
                ],
                details,
            };
        },
        ...
        renderCall(args, theme, context: RenderContext<UnifiedRenderState>) {
            ...
            const text =
                prepared && typeof prepared.text === "string"
                    ? prepared.text
                    : undefined;
            const key =
                text === undefined ? undefined : `${context.cwd}\0${text}`;
            ...
            return buildUnifiedEditCallComponent(
                component,
                text,
                theme,
                context.cwd,
        ...
        renderResult(
            ...
            _options,
            theme,
            context: RenderContext<UnifiedRenderState>,
        ) {
            const typed = result as {
                content: ToolContent;
                details?: UnifiedEditDetails;
            };
                    ...
                    changed =
                        setUnifiedEditPreview(
                            component,
                            {
                                ...
                                files: uniquePaths(
                                    typed.details.files.map(
                                        (file) => file.path,
                                    ),
                                ),
                                firstChangedLine:
                                    typed.details.firstChangedLine,
                            },
                            key,
                        ) || changed;
                }
            ...
            const resultComponent =
                (context.lastComponent as Container | undefined) ??
                new Container();
