/**
 * Finder subagent extension (sibling file): specialized spec + tool
 * registration, reusing the Subagent machinery from `lib/subagent.ts`.
 *
 * Registers one native pi tool:
 *   - `finder` : specialized code-search subagent (baked-in spec) for complex,
 *     multi-step codebase searches; child runs with read + bash only.
 */
